...
corpus = (
    courses_course[["id", "short_description"]]
    .dropna(subset="short_description")
...
```python
euclidean_distance_table_normalized = [
    [
        euclidean_distance_table[i][0],
        euclidean_distance_table[i][1],
        distance_to_similarity(euclidean_distance_table[i][2]),
    ]
    for i in range(len(euclidean_distance_table))
]
```
...
raw_data = pd.DataFrame(
    euclidean_distance_table_normalized, columns=["Corpus_X", "Corpus_Y", "distance"]
)
...
raw_data = pd.DataFrame(
    cosine_similarity_table, columns=["Corpus_X", "Corpus_Y", "distance"]
)
data = raw_data.pivot("Corpus_X", "Corpus_Y", "distance")
sns.heatmap(data)
```
